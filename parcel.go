...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&resParcel.Number, &resParcel.Client, &resParcel.Status, &resParcel.Address, &resParcel.CreatedAt)
		if err != nil {
			return nil, err
		}
		res = append(res, resParcel)
...
func (s ParcelStore) SetStatus(number int, status string) error {
	...
	if err != nil {
		return fmt.Errorf("не удалось обновить статус для %d: %w", number, err)
	}
	return nil
}
...
func (s ParcelStore) Delete(number int) error {
	...
	if currentStatus != ParcelStatusRegistered {
		return fmt.Errorf("Не возможно удалить строку с номером %d, статус посылки должен быть 'registered'", number)
	}
